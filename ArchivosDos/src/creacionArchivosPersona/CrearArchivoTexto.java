   ...
   public void agregarRegistros()
   {
      ...
      while ( bandera == true ) // itera hasta encontrar el indicador de fin de archivo
      {
         try // env�a valores al archivo
         {
            ...
            registro.establecerNombre( entrada.next() ); // lee el n�mero de cuenta
            System.out.println("Ingrese la edad");
            registro.establecerEdad( entrada.nextInt() ); // lee el primer nombre
            

            if ( registro.obtenerEdad() > 0 )
            {
               // escribe el nuevo registro
               salida.format( "%s %d\n", registro.obtenerNombre(),registro.obtenerEdad());
            } // fin de if
            else
            {
               System.out.println(
